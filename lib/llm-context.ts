...
NuvLoc 1.1.2 is a global dotnet tool (NuvyntraLabs.NuvLoc.Cli, command nuvloc) for sibling .resx hosts (MAUI, WPF, WinUI, Avalonia, Uno). Commands: init, update, agent add, plan, status, check, version. There is no nuvloc translate CLI command — the coding agent translates via /nuvloc.status and /nuvloc.translate. Config is i18n.json (platform, source, languages). languages and --lang must be BCP-47 cultures; casing is canonicalized (PT-br → pt-BR). Underscores, English source tags, and --lang not in i18n.json exit 2. Tested popular codes: es, fr, de, it, nl, ja, ko, zh-Hans, pt-BR, ar, hi, ru. Other BCP-47 codes also work. WinUI / Uno PRI .resw folders are out of scope. No vendor API key. Completeness is not correctness — review with a native speaker before ship. Interactive 4-hour nuget.org update check ([y/N], default no). Skip with --no-update-check or NUVYNTRA_NO_UPDATE_CHECK=1. Do not add NuvyntraLabs.NuvLoc.Cli as a PackageReference. Docs: ${siteConfig.url}/toolkits/nuvloc/
