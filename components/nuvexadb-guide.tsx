...
import { adjacentNuvexaPages, nuvexaGuideNav, type NuvexaGuidePage } from "@/content/nuvexadb-guide";
import { nuvexaDb, nuvexaDocsBase, nuvexaIntegrationHref } from "@/content/nuvexadb";
import { nuvexaGuideJsonLd } from "@/lib/json-ld";
import { cn } from "@/lib/utils";

export function NuvexaGuide({
  page,
  kind,
}: {
  page: NuvexaGuidePage;
  kind: "docs" | "integration";
}) {
  const { previous, next } = adjacentNuvexaPages(page.href);

  return (
    <main className="bg-background">
      <JsonLd data={nuvexaGuideJsonLd(page.title, page.description, page.href, kind)} />
      <div className="container max-w-7xl py-8 sm:py-10">
        <NuvexaGuideTabs active={kind} />
        <div className="mt-8 grid gap-10 lg:grid-cols-[220px_minmax(0,1fr)] xl:grid-cols-[220px_minmax(0,1fr)_180px]">
          <DocsSidebar groups={nuvexaGuideNav} currentHref={page.href} />
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-lavender-700 dark:text-lavender-300">
              {kind === "docs" ? "Documentation" : "Platform integration"}
            </p>
...
export function NuvexaGuideTabs({ active }: { active: "overview" | "docs" | "integration" }) {
  const tabs = [
    { href: "/nuvexadb/", id: "overview" as const, label: "Overview", icon: null },
    { href: `${nuvexaDocsBase}/`, id: "docs" as const, label: "White paper", icon: BookOpen },
    { href: nuvexaIntegrationHref, id: "integration" as const, label: "Platform integration", icon: Puzzle },
  ];

  return (
    <div className="flex flex-wrap gap-2" role="tablist" aria-label="NuvexaDB documentation">
      {tabs.map((tab) => {
...
function OnThisPage({ sections }: { sections: NuvexaGuidePage["sections"] }) {
  return (
