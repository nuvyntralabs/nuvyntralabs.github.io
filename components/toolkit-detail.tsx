    ...
    <main className="container max-w-3xl py-12 sm:py-16">
      ...
      {toolkit.notice ? (
        <aside className="callout mt-6 px-4 py-3">
          <p className="text-sm font-semibold text-foreground">{toolkit.notice.title}</p>
          <p className="mt-1 text-sm leading-relaxed text-muted-foreground">{toolkit.notice.text}</p>
        </aside>
      ) : null}
      ...
      <section className="mt-10">
        ...
        {toolkit.vscodeMarketplace ? (
          <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
            ...
            . The extension shells out to <code className="code-inline">maui-dev</code>{" "}
      ...
      <section className="mt-10">
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {toolkit.commands.map((command) => (
                <tr key={command.name} className="border-t border-border align-top">
                  <td className="px-3 py-2.5 font-medium text-foreground">
                    <a href={`#${commandAnchor(command.name)}`} className="hover:text-foreground">
        ...
        <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
          ...
          {toolkit.globalOptions.map((option, index) => (
            <span key={option}>
              {index > 0 ? ", " : ""}
              <code className="code-inline">{option}</code>
            </span>
          ))}
          .
        </p>
            ...
            <div className="mt-4 space-y-6">
              {group.commands.map((command) => (
                ...
                >
                  ...
                  <pre className="mt-2 overflow-x-auto rounded-xl bg-ink p-3 text-sm text-lavender-50">
