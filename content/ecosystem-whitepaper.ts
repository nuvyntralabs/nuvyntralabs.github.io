    ...
    blocks: [
      {
        ...
        rows: [
          ...
          [
            "NuvLoc CLI 1.1.2",
            "`nuvloc init` checks `i18n.json` and installs `/nuvloc.status` + `/nuvloc.translate`. `languages` must be BCP-47. The agent translates sibling `.resx` files. No vendor API key. Not a PackageReference.",
            "[NuvLoc](/toolkits/nuvloc/)",
          ],
        ],
      },
    ],
    ...
    blocks: [
      ...
      },
      {
        ...
        rows: [
          ...
          ["Localize", "NuvLoc: `i18n.json` (BCP-47 `languages`) + `nuvloc init` + `/nuvloc.translate` for sibling `.resx`. Native-speaker review before ship. Completeness is not correctness."],
          ["Ship", "Diagnostics + LeakAnalyser in Debug. Performance scoreboard. MauiDev `doctor` and `publish --validate` in CI. `nuvloc check --ci` on culture files."],
        ],
      },
      {
        type: "p",
