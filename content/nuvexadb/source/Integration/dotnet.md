...
| --- | --- |
| **`NuvexaDB-NuGet.zip`** | `Nuventra.NuvexaDB.1.0.4.nupkg` (optional: `Nuventra.NuvexaDB.1.0.4.snupkg` for symbols) |

Later tags use the same zip name; the nupkg version matches the tag (`Nuventra.NuvexaDB.1.0.4.nupkg` on v1.0.4).
...
`/path/to/unzipped-nuget` is the folder that contains `Nuventra.NuvexaDB.1.0.4.nupkg` (the zip root after you extract `NuvexaDB-NuGet.zip`).

Or add a local feed in `NuGet.config` and then `dotnet add package Nuventra.NuvexaDB --version 1.0.4`.
