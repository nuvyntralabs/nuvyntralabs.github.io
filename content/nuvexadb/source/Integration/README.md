...
The current published tag is **[v1.0.7](https://github.com/nuvyntralabs/NuvexaDB/releases/tag/v1.0.7)**. Later tags keep the same zip names. Pick the asset whose suffix matches your machine (`osx-arm64`, `win-x64`, `linux-x64`, …). Do not download every zip. The standalone `nuvexa` tool is `NuvexaDB-Cli` (`Nuventra.NuvexaDB.Cli`).
...
On [v1.0.7](https://github.com/nuvyntralabs/NuvexaDB/releases/tag/v1.0.7) those are the native zips that shipped, including Windows ARM64 and Linux ARM64.
