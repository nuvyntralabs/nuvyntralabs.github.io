...
export const packages: PackageDoc[] = [
  ...
  },
  {
    ...
    "title": "MVVMExpress",
    "subtitle": "Modular MVVM for MAUI — ViewModels, async state, Shell or page navigation, dialogs",
    "description": "A modular MVVM framework for .NET MAUI: observable models, async commands, bindable AsyncState, lifecycle-aware cancellation, typed Shell or page navigation, dialogs, toast, validation, pagination, forms, Reactive, and source generators. Public preview 0.5.0 — shipped APIs are the 1.0 contract; 1.0.0 waits on design-review sign-off.",
    ...
    "abstract": "MVVMExpress is the application shell for production MAUI apps. CommunityToolkit.Mvvm covers properties and commands, Prism.Maui covers page navigation (not Shell), and ReactiveUI covers observable pipelines. A field or enterprise app often needs all three plus bindable async state, lifecycle-aware cancellation, and typed navigation — without taking three overlapping frameworks. Core targets net10.0 and does not reference MAUI. 0.5.0 ships phases 1–5: Shell + page hosts, forms, Reactive, IOperationExecutor, scopes, source generators, persist/auth attributes, and CommunityToolkit adapters. Capability work (captive portal, HTTP cache, offline sync, form XAML, flags, deep links) stays in focused MauiEssentials plugins; MVVMExpress composes them through adapters.",
    "capabilities": [
      "ObservableModel, ViewModel lifecycle, and ViewModelCancellationToken cancelled on dispose.",
      "Sync and async commands with prevent / cancel-previous / queue / allow / replace, timeout, retry, debounce, and throttle.",
      "Bindable AsyncState<T> (Idle, Loading, Empty, Error, Offline, Unauthorized) and Outcome results.",
      "FormViewModel, FormField, dirty navigation guard, and undo/redo.",
      "IOperationExecutor, ICachedFetcher, IViewModelComposer, and IViewModelScopeFactory.",
      "MauiShellNavigator and MauiPageNavigator with typed records, IAcceptNavArgs<T>, and IAcceptNavQuery.",
      "URI stack: Stack, ModalStack, CanGoBack, PopToRoot, Replace, Reset. Multi-window via IWindowContext.",
      "IDialogs, MauiNotifier toast, DataAnnotations validation, PagedCollection<T>, and SearchQuery debounce.",
      "[Notify], command attributes, AddGeneratedViewModels, [PersistState], and [RequiresAuth].",
      "Reactive CombineLatest without System.Reactive; CommunityToolkitMessageHub adapter.",
